...
const ArchDiagram = () => (
  <div className="w-full h-full min-h-[260px] flex items-center justify-center p-6 select-none" aria-hidden="true">
    <div className="w-full max-w-xs flex flex-col gap-3 font-mono text-[11px]">
      {[
        { label: 'WhatsApp', color: 'text-lime', border: 'border-lime/30 bg-lime/5' },
        { label: 'FastAPI', color: 'text-zinc-300', border: 'border-zinc-700 bg-surface-2' },
        { label: 'LangGraph + Claude', color: 'text-zinc-300', border: 'border-zinc-700 bg-surface-2' },
      ].map((node, i) => (
        <div key={node.label} className="flex flex-col items-center gap-1">
          <div className={`w-full text-center border rounded-lg px-3 py-2 ${node.color} ${node.border}`}>
            {node.label}
          </div>
          {i < 2 && (
            <motion.div
      ...
      <div className="flex items-start gap-2">
        {['PostgreSQL', 'Redis + Celery'].map((label, i) => (
          <div key={label} className="flex flex-col items-center gap-1 flex-1">
            <motion.div
              animate={{ opacity: [0.3, 1, 0.3] }}
              transition={{ duration: 1.8, repeat: Infinity, ease: 'easeInOut', delay: 0.6 + i * 0.3 }}
    ...
    >
      <div className="grid grid-cols-1 lg:grid-cols-[60fr_40fr]">
        ...
        <div className="p-8 flex flex-col gap-4">
          ...
          <h3 className="font-display text-white" style={{ fontSize: '40px', lineHeight: 1.1 }}>
            {name}
          </h3>

          <p className="font-sans text-[#888] max-w-md" style={{ fontSize: '16px', lineHeight: 1.7 }}>
            {tagline}
          </p>
          ...
          <p className="font-mono text-[11px] text-muted mt-auto pt-2">
            Private — live with paying clients in Doha
          </p>
        </div>
